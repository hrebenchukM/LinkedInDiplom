(function () {
  ...
  window.removeHomePanelChatById = function (rawId) {
    const id = normalizeChatId(rawId);
    if (!id) return;
    const chats = loadChats().filter((c) => normalizeChatId(c.id) !== id);
    saveChats(chats);
    if (messagesPanelList && messagesPanelEmpty) {
      renderMessagesPanel();
    }
  ...
  function renderMessagesPanel() {
    ...
    chats.forEach((chat, index) => {
      ...
      function openChat() {
        window.location.href = `../chat/index.html?with=${encodeURIComponent(slug || "marcus")}`;
      }
  ...
  function applyRegisteredUserToDom() {
    ...
    const safeExternalAvatar =
      /^https:\/\//i.test(rawAvatar) && !rawAvatar.includes('"') && !rawAvatar.includes("'");
    const avatarUrl =
...
(function softNavPageReveal() {
  if (!document.body?.classList.contains("home-body")) return;
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      document.body.classList.add("home-page-reveal");
    });
  });

  function onAnimEnd(ev) {
    if (ev.target !== document.body || ev.animationName !== "homePageReveal") return;
    document.body.classList.remove("home-page-reveal");
