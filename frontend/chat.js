(function () {
  ...
  if (slugParam && readRemovedPeerSet().has(slugParam)) {
    ...
    window.location.replace(`./chat.html?with=${encodeURIComponent(next)}`);
    return;
  }
  ...
  document.querySelectorAll(".chat-list__row[data-peer] .chat-list__item").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (!id) return;
      window.location.href = `./chat.html?with=${encodeURIComponent(id)}`;
    });
  });
  ...
  function removePeerFromChatsAndRedirect(slug) {
    ...
    window.location.href = `./chat.html?with=${encodeURIComponent(next)}`;
  }
