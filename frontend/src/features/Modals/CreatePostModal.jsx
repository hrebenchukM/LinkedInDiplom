...
const CreatePostModal = ({ isOpen, onClose, user, onPostCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setError(t('home.modal.authRequired', 'Authentication required.'));
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('home.modal.createTitle', 'Create a post')}>
      <form onSubmit={handleSubmit}>
        ...
        >
          <img
            src={avatarSrc}
            alt={user?.name || t('post.userFallback', 'User')}
            style={{
              width: '48px',
              height: '48px',
              borderRadius: '50%',
          ...
          />
          <span className="create-post-modal__author">
            {user?.name || t('post.userFallback', 'User')}
          </span>
        </div>
        ...
        <div className="form-group">
          <textarea
            className="form-textarea create-post-modal__textarea"
            placeholder={t('home.modal.placeholder', 'What do you want to talk about?')}
            value={content}
        ...
        {imagePreview ? (
          <div style={{ marginBottom: '16px' }}>
            <img
              src={imagePreview}
              alt={t('home.modal.previewAlt', 'Preview')}
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="btn btn-primary" disabled={submitting}>
            {submitting
              ? t('home.publishing', 'Publishing…')
              : t('home.modal.publish', 'Publish')}
          </button>
        </div>
      </form>
    </Modal>
