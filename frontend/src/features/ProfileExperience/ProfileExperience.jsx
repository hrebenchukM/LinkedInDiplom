    ...
    <>
      <div className="experience-card">
        <div className="section-header">
          <h2>{t('profile.experience.title', 'Experience')}</h2>
        </div>

        {!hasExperiences && (
          <div className="experience-empty">
            <Briefcase size={24} />
            <h3>{t('profile.experience.emptyTitle', 'Add your first experience')}</h3>
            <p>{t('profile.experience.emptySub', 'Tell people about your professional background')}</p>
          </div>
        )}
            ...
            <div key={block.experience.id} className="experience-item">
              ...
              <div className="experience-content">
                ...
                <p className="experience-org">
                  {block.company?.name ?? t('profile.experience.companyFallback', 'Company')} · {block.experience.employmentType}
                </p>
                <p className="experience-date">
                  {block.experience.startDate} – {block.experience.endDate ?? t('profile.experience.now', 'Now')}
                </p>
              </div>
            </div>
          );
        ...
        <button className="btn-add" onClick={() => setIsModalOpen(true)}>
          {t('profile.experience.add', 'Add experience')}
        </button>
      </div>
