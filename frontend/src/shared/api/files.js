 ...
 */
export function getAssetUrl(url, fallback = '', cacheVersion) {
  const resolved = normalizeImageUrl(url);
  if (!resolved) return fallback;
  if (cacheVersion == null || cacheVersion === '') return resolved;
  const separator = resolved.includes('?') ? '&' : '?';
  return `${resolved}${separator}v=${encodeURIComponent(String(cacheVersion))}`;
}
