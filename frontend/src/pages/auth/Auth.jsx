...
import { login, register, googleLogin, facebookLogin } from '../../features/auth/authApi.js';
...
const AuthPage = () => {
  ...
  const handleSocialAuth = async (provider) => {
    ...
    try {
      if (canUseOAuthSdk(provider)) {
        const providerToken = await requestProviderToken(provider);
        const response = provider === 'facebook'
          ? await facebookLogin(providerToken)
          : await googleLogin(providerToken);
        completeAuth(mapAuthResponse(response));
        return;
      }

      if (shouldUseDemoSocialFallback(provider)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'signup' && formData.password !== formData.confirmPassword) {
      const mismatch = t('auth.passwordMismatch', 'Passwords do not match');
      setError(mismatch);
    ...
    <div className="auth-page">
      ...
      <div className="auth-card">
        <div className="auth-header">
          <h1 className="auth-title">{t('auth.welcomeTitle', 'Welcome to your')}</h1>
          <h2 className="auth-subtitle">{t('auth.welcomeSubtitle', 'professional community')}</h2>
        </div>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
              <path fill="#1877F2" d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.413c0-3.025 1.792-4.697 4.533-4.697 1.312 0 2.686.236 2.686.236v2.971H15.83c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/>
            </svg>
