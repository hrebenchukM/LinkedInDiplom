...
export function ProfilePage() {
  ...
  const initial = useMemo(() => {
    ...
    const seedName = session.user?.name || profile.name || "Student User";
    const parts = String(seedName).trim().split(/\s+/);
    const firstName = saved.firstName || parts[0] || "";
    const lastName = saved.lastName || parts.slice(1).join(" ");
    return {
      firstName,
      lastName,
      email: saved.email || "",
      specialty: saved.specialty || profile.headline || "",
      ...
      about: saved.about || profile.about || "",
  ...
  }, [profile.about, profile.city, profile.headline, profile.name, session.user?.name]);
  ...
  const [pendingAvatar, setPendingAvatar] = useState(initial.avatarDataUrl);
  ...
  function persist(nextForm, showHint = true) {
    ...
    updateProfile({
      name: fullName,
      headline: nextForm.specialty || profile.headline,
      city: nextForm.city,
      about: nextForm.about,
    });
    if (showHint) {
  ...
  async function onAvatarUpload(event) {
    ...
    if (file.size > 1_200_000) {
      setSaveHint(t("profile.hint.avatarTooBig", "Photo is too large (up to 1.2MB)."));
      return;
    }
    try {
      ...
      setSaveHint(t("profile.hint.avatarFail", "Failed to upload photo."));
    }
  }
          ...
          <article className="lk-card">
            ...
            >
              ...
              <label>
                ...
                <input type="email" value={form.email} onChange={(e) => patchForm({ email: e.target.value })} required />
              </label>
              <label>
              ...
              <button className="lk-cta" type="submit">
