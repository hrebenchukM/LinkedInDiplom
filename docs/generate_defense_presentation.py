...
FRONTEND_TREE_LINES = [
    "frontend/",
    "├── src/",
    "│   ├── app/           router, layout, theme, i18n, transitions",
    "│   ├── pages/         auth, home, network, vacancies, messages, admin",
    "│   ├── features/      auth, profile, jobs, messaging, network, content",
    "│   └── shared/",
    "│       ├── api/       HTTP-клієnt, tokens, refresh, paths",
    "│       ├── ui/        спільні компоненти",
    "│       └── lib/       helpers, pagination, session",
...
SLIDES = [
    {
        ...
        "facts": [
            "Проєкт: full-stack LinkedIn-like платформа з інтеграцією frontend ↔ backend",
            "Frontend розробники: Ямчук Тимур, Андрій Ротарь",
            "Стек: React 19, Vite 6, ASP.NET Core, PostgreSQL, JWT, SignalR",
            "Demo на захисті: andrii.rotar@gmail.com, admin@local.dev, verify:e2e 17/17",
        ],
    },
    {
        "type": "statement",
        ...
        "points": [
            ...
            "На захисті показуємо live-flow через реальний API: вхід → профіль → стрічка → мережа → вакансії → чати.",
        ],
        "metric": "6",
    ...
    },
    {
        ...
        "subtitle": "Реалізовано в поточній версії сайту на гілці merger",
        "footer": "Єдиний UX: темна/світла тема, 4 мови, плавні переходи та live backend API.",
        "items": [
            ...
            ("Анімації", "AnimatedOutlet — плавні переходи сторінок"),
            ("Пошук", "HeaderSearch по профілях і контенту"),
            ("Live API", "JWT, REST, refresh, SignalR messaging"),
            ("E2E verify", "17 автоматичних smoke-тестів інтеграції"),
        ],
    },
    {
        "type": "comparison",
    ...
    },
    {
        ...
        "points": [
            "pages/ — auth, home, network, vacancies, messages, notifications, admin",
            "features/ — API-виклики, mappers, enrich* для профілів і постів",
            "app/ — router, AppContext, Layout, theme, i18n, page transitions",
            "shared/api/ — http, tokens, refresh при 401, централізовані paths",
            "AppContext — auth, account, profile в одному провайдері замість legacy stores",
        ],
    ...
    },
    {
        ...
        "steps": [
            ("React/Vite SPA", "UI, routing, компоненти, theme, i18n"),
            ("AppContext", "auth, account, profile, token refresh"),
            ("API client", "REST + JWT + auto refresh при 401"),
            ("Facade.API", "BFF: auth, profile, content, network, jobs, messaging"),
            ("SignalR + E2E", "realtime чати, verify:e2e 17/17"),
        ],
        "note": "Frontend і backend працюють як одна система: SPA викликає REST API, SignalR для messaging, PostgreSQL на сервері.",
    },
    {
        "type": "frontend",
        "focus": "home",
        "title": "Маршрутизація frontend",
        "subtitle": "React Router 7, ProtectedRoute, JWT-сесія та live API",
        "routes": [
            ("/auth", "login, register, Google/Facebook OAuth"),
            ("ProtectedRoute", "захист /app/* маршрутів"),
            ("AppContext", "account, profile, token refresh при завантаженні"),
            ("/app/*", "home, network, vacancies, messages, profile"),
    ...
    },
    {
        ...
        "note": "На захисті показуємо live-сценарій: backend localhost:5000, frontend localhost:5173, demo-акаунт andrii.rotar@gmail.com.",
    },
    {
        "type": "admin",
        "focus": None,
        ...
        "note": "Admin-панель працює з реальним backend: admin@local.dev, JWT role Admin, /api/admin/* endpoints.",
    },
    {
        "type": "roles",
        "focus": None,
        ...
        "right": [
            ...
            "API-клієнт, backend-інтеграція та E2E verify (17/17).",
        ],
    },
    {
        "type": "feature",
        ...
        "points": [
            "Login/register через `/api/auth/*` з JWT access + refresh tokens.",
            "Google і Facebook OAuth кнопки; demo: andrii.rotar@gmail.com / LinkUpDemo2024!",
            "AppContext зберігає account і profile після входу; auto refresh токена.",
            "Auth-екран узгоджений з темною/світлою темою та i18n.",
        ],
    },
    {
        "type": "feature",
        ...
        "points": [
            "Список вакансій завантажується з `/api/jobs/vacancies` — реальні дані з backend.",
            "AI-рекомендації в sidebar: підбір за навичками профілю (Java, PostgreSQL) + VacancyDetailModal.",
            "Фільтри, збереження вакансій і apply flow підключені до jobs API.",
            "Кнопки «Детальніше» та «Бути серед кандидатів» — повний сценарій відгуку.",
        ],
    },
    {
        "type": "feature",
    ...
    },
    {
        ...
        "headline": "Чати — live messaging через SignalR; AI-асистент і дзвінки — UI-шар для наступних релізів.",
        "points": [
            "MessageFiltersModal: фільтри Unread, Contacts, Favorites, Spam, Archive, Drafts.",
            "SignalR hub для realtime повідомлень; unread, archive, mute, delete, share posts — live API.",
            "AI assistant і VoiceCallOverlay: повністю спроєктований UI-шар; на захисті — інтерактивні mock-моделі.",
            "Live WebRTC інтеграція запланована на наступні релізи; core messaging уже працює з backend.",
        ],
    },
    {
        "type": "feature",
        ...
        "points": [
            "Профіль синхронізується з `/api/profile/me`: headline, bio, avatar upload/delete.",
            "Skills, experience, education — CRUD через professional API; demo skills seeder.",
            "ProfileCompletion ring, resume upload, public profile `/app/profile/:userId`.",
            "Dark/light theme, i18n (uk/en/de/es) — єдиний візуальний стиль платформи.",
        ],
    },
    {
        "type": "feature",
        ...
        "headline": "Home — архітектурний hub після auth: зв'язує AppContext, routing і завантаження Feed.",
        "points": [
            "Стартова точка після login: orchestration layout, sidebar, швидкий доступ до модулів.",
            "AppContext передає account/profile у Home без prop drilling через legacy stores.",
            "Home ініціює завантаження стрічки з content API — перехід до найскладнішого модуля Feed.",
            "Єдиний UX-hub: мережа, вакансії, повідомлення, профіль з одного екрана.",
        ],
    },
    {
        "type": "feature",
        ...
        "headline": "Feed — найскладніший архітектурний модуль: глобальний стан, рендеринг і динамічні реакції.",
        "points": [
            "AppContext + локальний state: пости з content API, enrichPostsWithAuthors без full reload.",
            "Оптимізація рендерингу: PostCard, reactions, comments оновлюються точково після дії користувача.",
            "PostEngagement, PostCommentsPanel, PostShareMenu — повний цикл взаємодії з backend.",
            "CreatePost modal, rich text, media — технічно складніший блок, ніж статичні auth/profile форми.",
        ],
    },
    {
        "type": "feature",
        ...
        "headline": "Network: контакти, запити, suggestions і вкладка «Активність» з events.",
        "points": [
            "Вкладки «Нові контакти» та «Активність»: incoming/outgoing requests, people you may know.",
            "EventPanel: фільтри All/Upcoming/Online/In-person, discover events з API.",
            "AddContactSearch, ManageNetworkModal, networkPageCache для плавного UX.",
            "Groups, pages, company/event routes — `/app/groups/:id`, `/app/event/:id`.",
        ],
    },
    {
        "type": "feature",
        "focus": "home",
        "title": "Андрій Ротарь: API-клієнт і backend-інтеграція",
        "headline": "Frontend підключений до Facade.API — auth, profile, content, network, jobs, messaging.",
        "points": [
            "http.js: Bearer token, auto refresh при 401, централізовані paths і error handling.",
            "verify-e2e.mjs: 17 автоматичних smoke-тестів (auth, profile, feed, network, jobs, chats).",
            "SignalR messaging hub; enrich* helpers для профілів у постах, чатах, notifications.",
            "Docker Compose: Postgres + API :5000 + Vite :5173 — відтворюваний demo-стек.",
        ],
    },
    {
        "type": "matrix",
        ...
        "right": [
            ...
            "Roadmap: events seeding, OAuth production, integration tests.",
        ],
    },
    {
        "type": "final",
        "focus": None,
        "title": "Підсумок",
        "subtitle": "Full-stack MVP з live API — готовий продукт для захисту",
        "facts": [
            "LinkedIn Clone — full-stack професійна екосистема: frontend + backend + PostgreSQL.",
            "На захисті демонструємо live-flow: login → profile → feed → network → jobs → chat.",
            "Головний результат — інтегрований продукт з E2E verify 17/17 і зрозумілим UX.",
        ],
    },
]
...
MODULE_ICON_DATA = {
    ...
    "Live API": {"main": "API", "color_idx": 6},
    "E2E verify": {"main": "E2", "color_idx": 7},
...
FEATURE_TASK_META = {
    ...
    "чати": {
        ...
        "icon": "C",
        "color_idx": 5,
        "subtitle": "SignalR live + AI/WebRTC UI mock",
    },
    ...
    "головний екран": {
        ...
        "icon": "H",
        "color_idx": 0,
        "subtitle": "AppContext hub, routing, feed init",
    },
    "контентна стрічка": {
        "short": "Feed",
        "icon": "F",
        "color_idx": 3,
        "subtitle": "state, render opt., reactions",
    },
    "архітектура network": {
        "short": "Network",
        "icon": "N",
...
def draw_architecture(slide, data, idx, total):
    draw_base(slide, idx, total, data.get("focus"))
    add_title(slide, data["title"], "Frontend: AppContext, REST API, SignalR")
