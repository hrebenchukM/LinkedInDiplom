...
- admin users: `IUserAdminService` — list/get/lock/unlock/soft delete, `RevokeAllUserTokensAsync` при lock/delete
...
- platform admin: `AdminSoftDeletePostAsync` / `AdminRestorePostAsync` — без проверки владельца, отдельные методы
...
- platform admin: `AdminSoftDeleteVacancyAsync` / `AdminRestoreVacancyAsync` — без ownership; при admin delete/restore обновляется `UpdatedAt`
