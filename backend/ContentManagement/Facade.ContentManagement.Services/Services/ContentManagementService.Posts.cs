...
public partial class ContentManagementService
{
    ...
    public async Task<IReadOnlyCollection<PostDto>> GetFeedAsync(string userId, int limit = 50)
    {
        var posts = await _contentClient.Posts.GetFeedAsync(new GetFeedParameters
        {
            ViewerUserId = userId,
            Limit = limit
        });
